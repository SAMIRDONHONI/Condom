...
def approval():
  ...
  print ("""\033[1;37m--------------------------------------------------""")
  try:
    httpCaht = requests.get("https://github.com/SAMIRDONHONI/Condom/blob/main/Approval.txt").text
    if id in httpCaht:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {P}[SAMIR-XD]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
